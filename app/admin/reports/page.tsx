...
import { BarChart3, Download, Calendar, DollarSign, TrendingUp, ShieldAlert, FileText } from 'lucide-react'
...
export default function AdminReportsPage() {
  ...
  const handleExportToExcel = () => {
    ...
    // Buat header kolom baris pertama di Excel
    ...
    // Loop data penjualan masukkan ke baris-baris Excel
    report.sales.forEach((item: any) => {
      ...
      const modal = item.original_price || 0
      const jual = item.selling_price || item.final_price || 0
      const laba = jual - modal
      const metode = item.payment_method || '-'

      // Satukan baris teks dipisahkan tanda koma
      csvContent += `"${item.id}","${tanggal}","${kode}","${model}","${pembeli}",${modal},${jual},${laba},"${metode}"\n`
    })

    // Tambahkan baris total rekapitulasi di paling bawah Excel
    csvContent += `\n,,,,TOTAL OMSET,${report.totalModal},${report.totalOmset},${report.totalLaba},\n`

    // Logika download file otomatis dari browser browser
  ...
  return (
    <div className="p-4 sm:p-8 max-w-7xl mx-auto font-sans bg-slate-50 min-h-screen text-slate-900 rounded-3xl">
      ...
      {/* HEADER UTAMA */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8 border-b-2 border-indigo-100 pb-6">
        <div>
          <h1 className="text-2xl sm:text-3xl font-black text-slate-900 tracking-tight flex items-center gap-2">
            <BarChart3 className="w-8 h-8 text-indigo-600" />
            Laporan <span className="text-indigo-600">Keuangan & Omset</span>
          </h1>
          <p className="text-xs sm:text-sm text-slate-500 mt-1 font-medium">Pantau ringkasan margin keuntungan laba bersih showroom MotoSell secara periodik.</p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto">
          ...
          <div className="relative">
            <select
              ...
              className="pl-3 pr-8 py-2.5 bg-white border border-slate-200 rounded-xl text-xs sm:text-sm font-bold text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-600 transition appearance-none cursor-pointer"
            >
              {monthsList.map((m) => (
          ...
          <select
            ...
            className="px-3 py-2.5 bg-white border border-slate-200 rounded-xl text-xs sm:text-sm font-bold text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-600 transition cursor-pointer"
          >
      ...
      {loading ? (
        <div className="p-12 text-center text-slate-500 font-bold animate-pulse">Menghitung pembukuan neraca saldo laporan keuangan...</div>
      ) : (
        <>
